...
bootloader采用PIO的方式从硬盘中读取内核所在的各个扇区。首先，PIO操作相关的端口一共有8个，地址通常为`0x1F0~0x1F7`，具体端口如下：
...
readsect(void *dst, uint32_t secno) {
    ...
    outb(0x1F3, secno & 0xFF);      //0-7位扇区号
    outb(0x1F4, (secno >> 8) & 0xFF);   //8-15位柱面号低字节
    outb(0x1F5, (secno >> 16) & 0xFF);    //16-23位柱面号高字节
    outb(0x1F6, ((secno >> 24) & 0xF) | 0xE0);  //24-31位驱动器号
    /*
...
bootmain(void) {
    ...
    //先加载描述表（描述哪里的ELF文件分别应该加载多少字节，加载到内存的哪些位置
